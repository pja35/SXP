...
/**
 * Most part of this test is copied from:
 * crypto-153/core/src/test/java/org/bouncycastle/crypto/test/ElGamalEngineTest.java
 * @see https://www.bouncycastle.org/fr/latest_releases.html
...
public class ElGamalEngineTestK {
	...
	static private BigInteger getBigInt(int nbB){
		return 	new BigInteger(nbB, new Random());
	}

	static private BigInteger getBigInt(){
		int nbB = new Random().nextInt(33) + 32;
		return 	getBigInt(nbB);
	}
	...
	public void badParamsException1(){
		...
		exception.expectMessage("ElGamalPublicKeyParameters are required for encryption.");	
		engine.init(true, randPrivateParams);
	}
	...
	public void badParamsException2(){
		...
		ParametersWithRandom randPublicParams = new ParametersWithRandom(new ElGamalPublicKeyParameters(getBigInt(),
				new ElGamalParameters(getBigInt(), getBigInt())));
		exception.expect(IllegalArgumentException.class);
		exception.expectMessage("ElGamalPrivateKeyParameters are required for decryption.");
		engine.init(false, randPublicParams);
	}
